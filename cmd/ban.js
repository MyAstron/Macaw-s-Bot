module.exports ={
  use: async (Discord, message, Config, args) => {
    ...
    const error2 = new Discord.MessageEmbed()
      ...
      .setTitle(emojis.mcw.heh+"| You can't banned yourself! ")
      .setColor(colors.peachy)
    const error3 = new Discord.MessageEmbed()
      .setAuthor(message.member.user.tag, message.author.avatarURL({ dynamic: true}))
      .setTitle(emojis.mcw.knife+"| You do not have permissions to Ban people! ")
      .setColor(colors.mal)
      .setTimestamp()
    const error4 = new Discord.MessageEmbed()
      .setAuthor('Give Me the permissions for `Ban Members`', message.author.avatarURL({ dynamic: true}))
      .setTitle(emojis.mcw.angry+"| I do not have permissions to Ban people! ")
      .setColor(colors.mal)
      .setTimestamp()
    ...
    let razon = args.slice(1).join(' ');
      ...
      if (!banned) return message.channel.send(error1)
      if (!razon) return message.channel.send(error6)
      if (message.author === banned) return message.channel.send(error2)
      if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(error3)
      if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send(error4)
    const embedkick = new Discord.MessageEmbed()
      .setAuthor(banned.username+`#${banned.discriminator}`, message.guild.iconURL({ dynamic: true }))
      .setDescription("**"+banned.username+" • has been banned from "+message.guild.name+"!!** \n> **Razon**: "+razon)
      .setThumbnail(banned.avatarURL({ dynamic: true }))
      .setFooter("Moderation: "+message.member.user.tag, message.author.avatarURL({ dynamic: true })).setTimestamp().setColor(colors.peachy)

    try {
        message.guild.members.ban(banned, { reason: razon });
